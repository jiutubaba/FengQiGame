...
- 安全压缩：编辑器源码保持原样，单文件、多文件和旧版 `preloadCode` 接口都在生成最终 `preloadCode` 时按 Lua Token 清理注释、缩进、行尾空格和重复空白；字符串、必要间隔、Lua 5.4 局部变量属性及源码行数保留，词法不完整时不重写。页面同时显示源码与压缩后字节数，256 KiB 门禁统一按压缩结果计算；局部变量遮蔽的同名 `require` 不作为模块引用检查。
