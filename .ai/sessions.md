...
- 静态检查：原单文件 Lua 语法检查升级为 260ms 延迟的全工作区分析；静态字符串 `require` 会检查目标缺失、路径大小写和循环依赖，动态引用与未被 `main.lua` 加载的文件给出警告。问题在代码波浪线、行号、当前文件状态、目录文件 Badge 和工作区总状态同步展示，点击当前文件问题仍可定位行列；检查不阻止保存，游戏客户端仍负责最终编译与执行门禁。
