...
- 可达裁剪：发布打包复用 Lua AST 引用图，从 `main.lua` 递归保留有效直接静态 `require("文件名.lua")` 可达文件；注释引用和未引用文件只保留在 `preloadWorkspace`，不进入 `preloadCode`、256 KiB 限额与 bootstrap。入口可达的动态 `require` 或间接全局 `require` 会为兼容运行时加载而保守打包全部文件，不再误裁剪旧代码；动态全量模式下所有文件都须通过语法检查。61,458 字节的三文件注释引用样本发布产物为 0 字节，减少 61,458 字节（100%）。本次补强未部署生产，也未修改正式预加载数据。
- 验证：全量 5 个单元测试文件共 26/26、隔离 PostgreSQL 集成测试 17/17、生产构建、生产依赖审计和 `git diff --check` 通过；补充普通/长注释、等号层级、单双引号、转义引号、长字符串、中文、纯注释归空、Token 防拼接、256 KiB 边界、重复打包、失败保留旧产物、bootstrap 兼容、请求阶段不重复精简与 Token 序列回归。扩展语料复检发现 luaparse 对非法 shebang 的内部异常文本，现统一为明确 Lua 语法错误且后续解析不受污染。31,015 字节合成 Lua 压缩为 16,013 字节，减少 15,002 字节（48.4%），压缩前后均为 1,002 行。
- 合并与载荷：补强 PR #44 合并为 `main@ed5c76c`，PR 与 `main` 的质量/审计、隔离 PostgreSQL 两项 required checks 均成功，最终 CI 为 `32941529810`。`deploy-ed5c76c` 的源码包与 145 文件洁净构建包 SHA-256 分别为 `3c3d88ddf53afdd2dfd97691be3e3d84237c80858be53bed5dda61462798edd6`、`091f3c5280506d70682abab31f2df6ef36ea0900dd3394e5d77de56e49fed0dc`。
- 正式发布：发布前数据库备份 `fengqi-20260826-151545.dump` 与上传卷备份 `fengqi-uploads-20260826-151545.tar.gz` 均非空并完成私有 OSS CRC64 校验。第一次切换因源码复核把写入 `dist/` 和 `.release-commit` 后的根目录时间变化误判为漂移，自动回滚到 `c10ebd5` 且 app 恢复健康；改为忽略目录时间、继续逐文件校验后发布成功。正式 `.release-commit` 为 `ed5c76c`，镜像为 `sha256:7f807311864942f8af52a975afd838c934ce5b2df70d61ecc3625f422bd1dd16`，旧镜像标签 `pre-c10ebd5-20260826-152351` 与旧源码包已保留；数据库、上传卷、Caddy、`.env` 和网络未改变。
- 生产验收：镜像 revision、`001/002`、排行榜每日采集表、`players.rank_ban`、三容器 0 重启、app/db healthy、Caddy、备份 timer、公网 HTTP 308/HTTPS 200/WWW 保留 URI 301、安全头、健康接口、缺 Key 401、近 15 分钟 error/fatal 为 0、145 文件静态树及 3 个公网入口资源均通过。生产镜像内纯注释归空、字符串内 `--` 保留及 bootstrap 直接读取持久化字段烟测通过；未修改正式预加载数据，正式登录态配置页、真实地图 MD5/编译/执行与多人同步仍待联调。
