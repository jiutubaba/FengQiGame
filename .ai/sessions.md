# 近期开发归档

只保留最近 5 次重要开发任务。新增一条时删除最旧一条；稳定事实必须同步到系统文档。

## 2026-08-24 [Codex] FQ 预加载代码下发与权限收口

- 协议接入：`/api/fq/bootstrap` 从地图配置读取并下发 `preloadCode`，未配置时返回空字符串；地图配置继续使用既有保存接口和 `config` JSON，不新增协议或数据表。文本按 UTF-8 字节限制为 256 KiB，前端编辑器实时显示当前 UTF-8 容量与 256KB 上限。
- 权限边界：预加载代码具备远程执行能力，仅 `admin` 可修改；普通地图维护者即使拥有 `map.edit` 也只能只读查看，直接请求仍会收到 403。只消费礼包且提交 `includeMessages=false` 的客户端仅需 `game.gifts.read`，启用消息时才额外要求 `game.messages.read`。
- 客户端载荷：保留一次请求批量承载本局实际 UID 的协议，24 人仅为通用接口上限；移除存档读取时间、保存正文回显、玩家 upsert 回显、指标时间、榜单候选元数据、礼包名称、消息创建时间及 ACK 详情。榜单只下发游戏展示和当日采集所需字段；投递支持 `includeMessages: false` 完全跳过消息查询。
- 验证边界：`npm run check` 通过（18 个必需入口、23 份文档、2 份生成事实、13 项单元测试与生产构建），`npm run audit:prod` 为 0；本地合成管理员与空配置完成桌面、390px 窄屏浏览器验收，均显示 `0/256KB`，窄屏无横向溢出且控制台无警告或错误。全链路集成用例已补齐空配置、管理员保存下发、普通维护者拒绝、256 KiB 边界接受和超限拒绝；本机没有可安全使用的隔离测试库，集成用例未执行。后台未正式发布，地图包未构建/发布，真实客户端尚未联调。

## 2026-08-17 [Codex] 登录页 Galaxy 动态视觉升级

- 视觉与边界：登录页左侧由浅色网格升级为深石墨动态星海，保留品牌主标题、地图隔离说明、返回首页与安全会话状态；右侧暖白登录表单、账号密码交互、错误反馈、跳转和认证接口均未改变。视觉规范明确该深色画布只属于登录品牌入口，不扩散到登录后业务工作区。
- 组件与性能：新增基于 React Bits Galaxy 思路和 `ogl` 的 WebGL 组件，使用稳定的标量依赖避免表单重渲染反复创建上下文；通过 `ResizeObserver`、`IntersectionObserver`、页面可见性、`prefers-reduced-motion` 与 680px 移动断点控制渲染，隐藏的窄屏画布缩至 1×1，鼠标交互由父级承接且不拦截链接或表单。WebGL 不可用时保留静态星点背景。
- 验证：`npm run check` 通过（18 个必需入口、23 份文档、2 份生成事实、13 项单元测试与生产构建），`npm run audit:prod` 为 0，`git diff --check` 通过。本地浏览器完成 1440×900 桌面与 390×844 窄屏验收：桌面 Galaxy 状态为 ready 且仅有一个 755×900 画布；窄屏左侧隐藏、画布为 1×1、页面无横向溢出，控制台无警告或错误；媒体仿真确认减少动态偏好生效。预览使用 Vite 静态构建，真实 API 未启动，未提交真实账号或执行登录。
- 合并与载荷：PR #34 合并为 `main@82f614b`，PR 与 `main` CI 运行 `31979948451` 的质量/审计和隔离 PostgreSQL 两项 required checks 均通过。GitHub 非正式发布 `deploy-82f614b` 的源码包 SHA-256 为 `1e1da887430ccd08dcdf99ac3bef93bc67d437b354e61c7034cca119e419cc2d`；发现本机既有 `dist` 混有历史未引用哈希文件后，没有沿用该目录，而是改在全新隔离目录构建 145 个当前文件，正式构建包 SHA-256 为 `741917453dc0500df9b387712b740678354c9ce745a61c582f74b6a14d6df1a3`。
- 正式发布：发布前数据库备份 `fengqi-20260817-074840.dump` 与上传卷备份 `fengqi-uploads-20260817-074841.tar.gz` 均非空并完成私有 OSS CRC64 校验。云助手 app-only 发布后，正式 `.release-commit` 为 `82f614b`，镜像为 `sha256:77862f1739b56316190463a0f672ffeab3e9cfca3d45800a97e02ff65e88ce24`；旧镜像标签 `pre-10f453c-20260817-075112`、校正前健康镜像标签 `pre-dist-cleanup-82f614b-20260817-075356` 与源码备份 `source-pre-10f453c-20260817-075112.tar.gz` 已保留，未改变 PostgreSQL、上传卷、Caddy、`.env` 或网络。
- 生产验收：`001`、`002` 迁移登记与关键表/字段存在，app/db healthy、Caddy running、备份 timer active/enabled，容器内与公网健康接口 200；公网 HTTP 308、`www` 保留 URI 301、四项安全头、缺 Key 401、新前端四个入口资源哈希和近 15 分钟 app error/fatal 为 0 均通过。内置浏览器能导航至 `https://fengqigame.com/login` 并读取标题“风起游戏”，但 DOM/Canvas 与控制台读取连续超时；正式 WebGL 可见状态和控制台未完成，未将 HTTP、哈希或本地视觉验收冒充为正式浏览器交互验收。

## 2026-08-12 [Codex] 后台模块化更新正式发布

- 代码门禁：前端信息架构、按需加载与地图路由领域化改动由 PR #32 合并为 `main@10f453c`。PR 首轮隔离 PostgreSQL 测试发现 `files.js` 拆分时漏迁移上传扩展名黑名单和内联图片类型集合，文件上传返回 500；原常量原样补回后 PR 与 `main` CI 均通过，最终运行 `31612077849` 的质量/审计与隔离 PostgreSQL 两项检查成功，集成测试 17/17 通过。
- 载荷与备份：从干净合并提交重新执行 `npm ci`、`npm run check` 和 `npm run audit:prod`，13 项单测、生产构建与 0 生产漏洞审计通过。GitHub 非正式发布 `deploy-10f453c` 保存源码包和正式构建包，SHA-256 分别为 `e2e16d1c48670bcdc09e0f44d911548aa5950f5845fde750a2d6b61e9ab231d4`、`4c94ed5dff7dfb5ae7a4bd228ed37a9b0941aaf595cff399be7dd6e84de3d846`。发布前数据库备份 `fengqi-20260812-232707.dump` 与上传卷备份 `fengqi-uploads-20260812-232708.tar.gz` 均为非空并完成私有 OSS CRC64 校验。
- 正式发布：服务器下载载荷并复核 SHA-256，以旧正式镜像为基底替换 `server` 与新鲜 `dist`，只重建 app。正式 `.release-commit` 为 `10f453c`，镜像为 `sha256:ee86044631f1c7ff7aee8226907772d9a4102be5caf5e7ceb8e18d23dd9b801a`；旧镜像标签 `pre-ce7cec1-20260812-232910` 和源码备份 `source-pre-ce7cec1-20260812-232910.tar.gz` 已保留，未改变 PostgreSQL、上传卷、Caddy、`.env` 或网络。
- 生产验收：app/db healthy、Caddy running、三容器 0 重启，容器内健康 200，`001`、`002` 迁移与目标表/字段存在，备份 timer active/enabled，近 15 分钟 app error/fatal 为 0。公网 HTTP 308、HTTPS 200、`www` 保留 URI 301、安全头、缺 Key 401 和 `FQ_MISSING_API_KEY` 均通过；首页、主 JS、主 CSS 哈希与本次本机构建一致。Chrome 扩展侧连续两次超时，正式页面的真实浏览器可见状态与控制台未完成，未将 HTTP 与哈希验收冒充浏览器交互验收。

## 2026-08-12 [Codex] 地图后台路由领域化代码治理

- 模块边界：`server/routes/maps.js` 从 2813 行业务混合文件收口为 31 行组合入口；原有实现按地图生命周期、玩家与消息、排行榜、风控、礼包与群抽、运营资源、文件、API Key 拆入 `server/routes/maps/` 的 9 个领域模块，仅将跨领域复用的 ID 校验与分页解析保留在 `shared.js`。没有新增依赖、兼容层、API 或业务行为。
- 合约保持：组合入口仍挂载于 `/api/maps`，各领域注册顺序与拆分前一致；对拆分前后 Express 路由清单逐项比较，61 条路由的方法、路径、顺序和中间件数量完全一致，模块入口可独立导入。权限中间件、事务范围、SQL、响应数据形状和审计动作均保留在原业务流程内。
- 文档治理：架构、源码上下文路由、代码组织规范和各领域主要证据已同步到新目录；自动事实生成器补齐对组合路由同名目录的递归扫描，生成事实仍包含 98 条 API，其中 61 条地图路由准确指向各领域文件；模块边界待办已完成。
- 回归修复：最终复核时，本地预览暴露出趋势数据缺少当前指标字段时 SVG 圆点纵坐标为 `NaN`；`MetricsPanel` 改为复用同一组已归一化数值计算纵坐标，保持缺失字段按既有口径归零。重新打开地图指标页后，8 个趋势节点坐标均为有限数值，控制台无警告或错误。
- 验证：`npm run check` 通过（18 个必需入口、23 份文档、13 项单元测试与生产构建），`npm run audit:prod` 为 0，`git diff --check` 通过。隔离集成测试在执行用例前因本机环境阻塞：仓库 `.env` 为 Docker 模式但 Docker 引擎未启动，本机 PostgreSQL 使用不同凭据，连接 `127.0.0.1:55432/fengqi_test` 认证失败，17 项用例未执行；未重置数据库凭据、未连接正式库、未发布正式环境。本地指标页预览已按前端交付规则打开并保留。

## 2026-08-12 [Codex] 后台前端信息架构、恢复能力与按需加载优化

- 信息架构与视觉：保留既有暖白、灰阶和单一蓝色的运营控制台基线；地图功能导航按“数据监控 / 玩家运营 / 地图管理”分组，数据中心按核心经营、用户流动、留存复玩重排指标层级，并提高正文、表格、说明与操作控件的最小字号。地图卡片改为语义化链接，窄屏登录页恢复返回首页入口，未增加装饰性模块或改变业务含义。
- 交互与可访问性：路由和地图功能写入动态页面标题；移动抽屉增加滚动锁定、背景 `inert`、初始焦点、Tab 焦点循环、Escape 关闭与焦点归还；账户菜单补齐外部点击、Escape、上下方向键及 Home/End。地图中心、排行榜和礼包关键视图/搜索/选择状态写入 URL，刷新或分享链接可恢复；搜索框、视图切换与地图入口补齐可访问名称和状态语义。
- 数据恢复与性能：地图中心及地图工作台各业务域的首屏失败显示可重试错误，刷新失败则保留上次成功数据并显示持久提示。`src/pages/MapWorkspace.jsx` 从 4832 行收口为 185 行壳层，指标、配置、玩家、排行榜、风控、礼包与资源面板拆入 `src/pages/map-workspace/` 并按当前功能懒加载；所有页面路由改为懒加载，移除未再使用的 `Noto Serif SC` 依赖。未改变 API、权限、事务或服务端校验。
- 前端交付规则：`.ai/quality.md` 明确要求所有涉及 `src/**` 页面、组件或样式的任务在代码与检查完成后启动本地预览、打开对应页面并保留浏览器标签页；真实 API 不可用而使用合成数据时必须标注，不能用构建通过代替已打开预览。字体规则同步为当前实际使用的单一 `Noto Sans SC Variable`。
- 验证：`npm run check` 通过（18 个必需入口、23 份文档、13 项单元测试与生产构建），`npm run audit:prod` 为 0。本地浏览器使用合成数据完成桌面与 390px 窄屏验收，覆盖地图中心、指标分组、排行榜与礼包 URL 恢复、移动抽屉和账户菜单键盘焦点、刷新失败保留旧数据及登录页返回入口，浏览器控制台无警告或异常。本机真实 API 因当前数据库主机配置为容器名 `db` 而无法启动（`ENOTFOUND db`），因此未连接真实 PostgreSQL、未运行隔离集成测试、未发布正式环境。
