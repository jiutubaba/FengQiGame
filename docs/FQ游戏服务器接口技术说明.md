...
每个 UID 最多返回最早的 100 条消息，并返回该地图、环境和玩家下全部正数礼包资格。存档封禁只让消息为空，不影响礼包资格：
...
消息必须先幂等写入玩家完整存档，保存成功后 ACK。礼包是当局实时资格快照，只覆盖本局内存值，不写入玩家存档，也没有 ACK 接口；下一局重新查询，`value > 0` 表示激活。《沧澜》只识别现役福利礼包名称，因此用于激活的 `gift_key` 必须与对应礼包名称完全一致。
...
礼包由后台维护当前值。设置正数会创建或覆盖资格，设置 `0` 会删除资格；游戏每局通过 `/api/fq/deliveries/query` 读取完整正数快照。同一个礼包不会跨局累计，重复查询也不会改变数值。
