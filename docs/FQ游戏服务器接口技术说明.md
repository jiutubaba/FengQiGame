...
`preloadCode` 始终返回字符串，空字符串表示无预加载操作。该字段具备游戏端代码执行能力，仅管理员可以修改；后台编辑器完整保留含注释的原始 Lua 文件，管理员保存时从 `main.lua` 出发递归解析有效直接静态 `require("文件名.lua")`，只把入口可达文件按 Lua 词法生成并持久化为去注释产物。注释中的引用和未被入口加载的文件仍可编辑、审查和回滚，但不进入发布产物、256 KiB 限额或 bootstrap 载荷；编辑器将这类文件以中划线和“未加载”标记，并从语法、引用及问题总数中排除，重新被有效引用后恢复检查。纯空白、纯注释或没有有效可达业务代码的产物归一为空字符串。进入本次产物的文件存在语法错误时拒绝本次发布；存在动态 `require` 或间接使用全局 `require` 时为兼容运行时加载而保守打包并检查全部源码文件，仍会精简注释和无意义空白。打包或精简失败、产物超过 256 KiB 都会保留上一份有效产物。bootstrap 只读取已发布产物，不在请求阶段重复打包或精简。游戏端协议不变，仍须在共享业务加载前对实际收到的文本计算摘要、以文本模式编译并执行一次，编译或运行失败时不得进入 FQ 在线态。多人地图还必须把代码摘要纳入全员启动状态校验。预加载代码只用于定义或覆盖开局所需的 Lua 数据与函数，不得创建世界对象、启动计时器、发送同步消息，也不得依赖本地玩家、客户端时间或其他会造成各端结果不同的输入。
...
后台为每个榜单选择以下分数更新策略，每日限制按 FQ 服务器北京时间自然日计算：

- `latest`：当日首次样本覆盖分数、附属字段和达成时间；
- `best`：当日首次样本严格更优时才覆盖；
- `realtime_latest`（实时更新样本）：每次有效上报覆盖分数、附属字段和达成时间；
- `realtime_best`（实时更新更优样本）：每次有效上报严格更优时才覆盖。

升序榜数值更小为更优，降序榜数值更大为更优；更优策略遇相同成绩保留原分数、附属字段和达成时间。玩家名称随有效采集刷新。
...
- `submittedTodayUids` 在每日策略下返回请求 UID 中今日已采集玩家；实时策略固定返回空数组，客户端不应自行施加每日一次限制；
...
每日策略下，同榜单同 UID 每个北京时间自然日只接受首次提交；并发请求由独立每日采集记录原子去重，失败或结果不确定时重试会跳过已采集 UID。实时策略接受同日重复上报，也保存每日采集事实；切回每日策略后，当天已采集的 UID 会被跳过。后台移除候选不会删除采集事实。实时覆盖按服务端写入顺序处理，不提供 requestId 或版本保护；客户端应串行发送，避免迟到或重试的旧样本覆盖较新成绩。实时更优使用原子比较，不会被较差或相等分数覆盖。被后台标记为排行榜封禁的玩家不会进入候选排名和后续新快照；已经发布的历史快照不会被改写。
...
- [ ] 每日策略下同一 UID 每个北京时间自然日只接受首次样本，并发请求只接受一次；实时策略允许重复上报，更优策略按升降序严格比较；
