...
- PostgreSQL BIGINT/NUMERIC 字段可能以 JSON 字符串返回；消息 ID、礼包发放 ID 等应按不透明字符串处理。
...
| `game.players.write`      | POST | `/api/fq/players/upsert`                            | 新增或更新玩家               |
...
| `game.leaderboards.read`  | POST | `/api/fq/leaderboards/:leaderboardKey/query`        | 读取前 100 名和玩家当前名次  |
| `game.leaderboards.write` | POST | `/api/fq/leaderboards/:leaderboardKey/entries`      | 批量更新实时排行榜           |
...
| `game.gifts.read`         | POST | `/api/fq/gifts/:grantId/ack`                        | 确认礼包已写入玩家存档       |
...
该接口按地图、环境、UID upsert，并刷新最后活跃时间。
...
- `latest`：每次上报覆盖分数、附属字段和达成时间；
- `best`：只在新分数严格优于当前记录时覆盖分数、附属字段和达成时间；同分或更差只更新玩家名。升序榜数值更小为更优，降序榜数值更大为更优。
...
- 返回 `entries` 前列、`playerRanks` 中指定 UID 的当前名次、`totalEntries` 和生成时间；
- 条目包含 `rank`、`uid`、`name`、`gameLevel`、`score`、`gameCount`、`metadata`、`achievedAt` 与北京时间展示字段 `achievedAtText`；
- 被当前环境标记为排行榜封禁的玩家不参与总数、排名或指定 UID 查询。
...
同一个榜单中的同一 UID 不会新增重复条目，具体覆盖规则由榜单的 `latest/best` 模式决定。被后台标记为排行榜封禁的玩家不会进入实时排名和后续新快照；已经发布的历史快照不会被改写。
...
```text
拉取 pending -> 游戏内写入成功 -> ACK
```

### 10.1 拉取消息

```http
GET /api/fq/players/:uid/messages
```
...
```json
{
  ...
  "data": [
    {
      "id": "42",
      "gift_key": "maintenance_gold",
      "name": "维护补偿",
      "quantity": "100.0000",
      "boolean_value": false,
      "granted_at": "2026-07-16T10:45:00.000Z"
    }
  ]
}
```
...
```json
{
  ...
  "data": {
    "id": "42",
    "delivered_at": "2026-07-16T10:46:00.000Z"
  }
}
```

发放不存在、UID 不匹配、环境不匹配或已经 ACK 时：

```text
404 GIFT_NOT_FOUND
```

游戏侧必须按 `grantId` 自身幂等。只有礼包已经成功进入玩家存档后才能 ACK。
...
| 埋点累加                 | 否             | 重试会重复增加计数                                           |
| 消息/礼包 ACK            | 条件允许       | 本地先按 ID 幂等；不确定时重新拉取 pending，不循环重试 404   |
...
截至 2026-07-21，纯 Lua 适配与落地战力排行榜代码自检已通过，但本机私有配置尚未创建，真实游戏仍未完成 `test` 环境验收。现有三套 Key 只含玩家写入和存档读写权限，而《沧澜》启动链还会读取消息、礼包和落地战力榜，并在英雄落地后写榜；联调前必须按最小权限补充 `game.messages.read`、`game.gifts.read`、`game.leaderboards.read` 与 `game.leaderboards.write`。详细交接见 [沧澜项目 AI 技术交接](沧澜项目AI技术交接.md)。
...
- [ ] 消息和礼包只在游戏内写入成功后 ACK；
- [ ] `landing_power_v1` 使用 `best` 模式，赛前 Top100/本人名次可读，英雄落地后只批量上报一次；
- [ ] 消息 ID、礼包发放 ID、风险 eventId 在游戏侧幂等；
- [ ] 日志和埋点不盲目重试；
