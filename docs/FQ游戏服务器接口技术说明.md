...
`preloadCode` 始终返回字符串，空字符串表示无预加载操作。该字段具备游戏端代码执行能力，仅管理员可以修改；后台编辑器保留含注释的原始 Lua 文件，管理员保存时按 Lua 词法生成并持久化去注释产物，纯空白或纯注释产物归一为空字符串。精简失败或精简后超过 256 KiB 会拒绝本次发布并保留上一份有效产物；bootstrap 只读取已发布产物，不在请求阶段重复精简。游戏端协议不变，仍须在共享业务加载前对实际收到的文本计算摘要、以文本模式编译并执行一次，编译或运行失败时不得进入 FQ 在线态。多人地图还必须把代码摘要纳入全员启动状态校验。预加载代码只用于定义或覆盖开局所需的 Lua 数据与函数，不得创建世界对象、启动计时器、发送同步消息，也不得依赖本地玩家、客户端时间或其他会造成各端结果不同的输入。
