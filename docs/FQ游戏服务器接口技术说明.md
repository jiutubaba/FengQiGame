...
截至 2026-07-21，排行榜代码已部署，`test` 环境已创建降序 `best` 的 `landing_power_v1`；新测试 Key 只授予玩家写入、存档读写、消息/礼包读取和排行榜读写 7 项权限，本机私有配置已创建并由 Git 忽略。真实 API 已验证空榜查询、批量写入、低分不覆盖和高分更新，验收探针已删除。真实游戏仍未完成 `test` 环境验收；`release/lobby` 保持原配置，测试构建通过后再决定推广和正式 Key 轮换。详细交接见 [沧澜项目 AI 技术交接](沧澜项目AI技术交接.md)。
