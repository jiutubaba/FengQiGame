...
| `game.messages.read` + `game.gifts.read` | POST | `/api/fq/deliveries/query`             | 批量拉取待送达消息与礼包     |
...
每个 UID 最多返回最早的 100 条消息和 100 条礼包；存档封禁玩家返回空数组：
...
消息和礼包都必须先幂等写入玩家完整存档，保存成功后才分别 ACK。
...
截至 2026-07-21，批量开局、每日首次采集和人工发布快照协议已在源码中更新；部署前必须先应用 `006_leaderboard_publication_and_daily_collection.sql`，再为每个启用榜单人工发布一份新快照。旧版实时读榜验收不能代替本轮真实环境回归，游戏端已修改，未测试。详细交接见 [沧澜项目 AI 技术交接](沧澜项目AI技术交接.md)。
