...
ICP 网站备案已通过；公网 DNS 恢复并通过 HTTPS 验收前，应由部署负责人提供临时联调入口。不要把 ECS 公网 IP 硬编码为正式地址，因为 HTTPS 证书和 Host 都绑定正式域名。
...
同一个榜单中的同一 UID 会被更新，不会新增重复条目。被后台标记为排行榜封禁的玩家不会进入实时排名和后续新快照；已经发布的历史快照不会被改写。
