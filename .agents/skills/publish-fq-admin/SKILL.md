...
- 核对 `docs/协作开发与交接.md` 的多人发布职责：本次指定发布人使用自己的已授权云身份，另一维护者及其 AI 不并行发布；遵守当前 PR 审核门禁，不借用他人身份批准。
...
将双备份、切换、验证与失败回滚收口在同一个云助手远端脚本内，进入写入阶段前执行以下互斥前缀；后续步骤必须保持文件描述符 9 打开。其他发布或回滚入口使用同一路径，不删除锁文件以“解锁”。先确认 `/opt/fengqigame` 是本项目部署目录。

```bash
set -euo pipefail
exec 9>/opt/fengqigame/.deploy.lock
flock -n 9 || { printf '%s\n' '已有正式发布执行中，停止本次发布。' >&2; exit 75; }
```

远端异常退出通过既有回滚处理结束后才释放锁。云助手本地超时或连接失败不等于远端已停止；先查询原 invocation 状态，不重复投递或启动接管发布。锁是参与该规范的入口间互斥，不替代云端权限控制。
